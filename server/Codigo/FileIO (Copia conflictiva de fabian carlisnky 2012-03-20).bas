...
Function EsDios(ByVal Name As String) As Boolean
    If UCase$(Name) = "MARIUS" Or _
        UCase$(Name) = "RUSTY" Or _
        UCase$(Name) = "FEDUDOK" _
    Then
        EsDios = True
        Exit Function
    End If
...
Function EsVIP(ByVal Name As String) As Boolean
    Dim i As Long, numVips As Long
    Dim leer As New clsIniReader
    
    leer.Initialize App.Path & "\server.ini"
    numVips = leer.GetValue("VIPS", "Nums")
    For i = 1 To numVips
        If UCase$(Name) = UCase$(leer.GetValue("VIPS", "VIP" & i)) Then
            EsVIP = True
            Exit Function
        End If
    Next i
...
For Hechizo = 1 To NumeroHechizos
    ...
    If Hechizos(Hechizo).Metamorfosis = 1 Then
        ...
        Hechizos(Hechizo).Asusta = val(leer.GetValue("hechizo" & Hechizo, "asusta"))
    End If
...
On Error GoTo man
    ...
    For map = 1 To NumMaps
        ...
        If map = Banderbill.map Or map = Arghal.map Or _
           ...
           map = NuevaEsperanzapuerto.map Then
