...
'############################################################
    ...
    For LoopC = 1 To LastUser
        If UserList(LoopC).flags.UserLogged And UserList(LoopC).Counters.Trabajando > 0 And (Not UserList(LoopC).flags.Privilegios And PlayerType.vip) Then
            If Not UserList(LoopC).flags.CentinelaOK Then
                ...
                If CentinelaNPCIndex Then
                    ...
                    Call WriteConsoleMsg(1, LoopC, "El centinela intenta llamar tu atención. ¡Respóndele rápido!", FontTypeNames.FONTTYPE_BROWNB)
...
On Error GoTo Error_Handler
    ...
    If Not UserList(Centinela.RevisandoUserIndex).flags.CentinelaOK Then
        ...
        Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(1, "Servidor> El centinela ha baneado a " & Name, FontTypeNames.FONTTYPE_SERVER))
...
'############################################################
    ...
    If UserIndex = Centinela.RevisandoUserIndex Then
        If Not UserList(UserIndex).flags.CentinelaOK Then
            ...
            Call WriteConsoleMsg(1, UserIndex, "El centinela intenta llamar tu atención. ¡Respondele rápido!", FontTypeNames.FONTTYPE_BROWNB)
        Else
            'Logueamos el evento
...
'############################################################
    ...
    Else
        ...
        Else
            ...
            Call WriteConsoleMsg(1, Centinela.RevisandoUserIndex, "¡" & UserList(Centinela.RevisandoUserIndex).Name & ", tienes un minuto más para responder!", FontTypeNames.FONTTYPE_BROWNB)
            Call FlushBuffer(Centinela.RevisandoUserIndex)
        End If
    End If
End Sub
